...
/*
	ANA GABRIELA DE CHAVES
	<ECOVILLE, QUITA FEIRA, NOITE>
*/
...
namespace Prova1
{
	...
	class Controle
	{
		...
		public int GetNumeroQuiosquesApenasNoite()
		{
			...
			foreach(Quiosque quiosqueNoite in Quiosques)
			{
				if(quiosqueNoite.Horario == "Noite")
				{
					 numeroQuiosquesApenasNoite = numeroQuiosquesApenasNoite + 1;
				}
			}
		...
		public string GetEmpresasSalgadoManha()
		{
			...
			foreach (Quiosque salgadoManha in Quiosques)
            {
                if (salgadoManha.Horario != "Noite" && salgadoManha.ServeSalgado == true)
                {
                     empresasSalgadoManha += salgadoManha.Empresa + ", ";
                }
		...
		public string GetAlimentoMaisPreferido()
		{
			...
			foreach (Bloco bloco in Blocos)
			{
				...
				else if (bloco.AlimentoPreferido == "Salgado")
				{
					salgado++;
				}
